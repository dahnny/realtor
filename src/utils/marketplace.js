...
import approvalProgram from "!!raw-loader!../contracts/realtor_contract_approval.teal";
import clearProgram from "!!raw-loader!../contracts/realtor_contract_clear.teal";
import {base64ToUTF8String, utf8ToBase64String} from "./conversions";

class Property {
    constructor(image, description, location, sellprice, sale, likes, appId, owner) {
...
export const createPropertyAction = async (senderAddress, property) => {
    ...
    let note = new TextEncoder().encode(realtorNote);
    let image = new TextEncoder().encode(property.image);
    let description = new TextEncoder().encode(property.description);
    let location = new TextEncoder().encode(property.location);
    let sellprice = algosdk.encodeUint64(property.sellprice);

    let appArgs = [image, description, location, sellprice]
...
// LIKE PROPERTY: Group transaction consisting of ApplicationCallTxn 
export const likeAction = async (senderAddress, property) => {
    console.log("liking property...");
  ...
  export const sellAction = async (senderAddress, property) => {
...
const getApplication = async (appId) => {
    try {
        ...
        if (getField("SELLPRICE", globalState) !== undefined) {
            sellprice = getField("SELLPRICE", globalState).value.uint
        }

        if (getField("SALE", globalState) !== undefined) {
            sale = getField("SALE", globalState).value.uint
        }
        ...
        return new Property(image, description, location, sellprice, sale, likes, appId, owner)
    } catch (err) {
        return null;
    }
}
